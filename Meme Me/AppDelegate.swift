...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "MemeClass")
        do {
            if let results = try CoreDataStackManager.sharedInstance().managedObjectContext.fetch(fetchRequest) as? [MemeClass] {
                storedMemes = results
                memes = [MemeStruct]()
